...
# --- Fused Triton MLP kernel (from PR #1072 by @vimeto) ---
# Fuses: x @ up_w.T -> leaky_relu(neg_slope) -> square into one kernel pass
# Falls back to standard path on ROCm or if Triton < 3.3
HAS_FUSED_MLP = False
IS_ROCM = hasattr(torch.version, 'hip') and torch.version.hip is not None
_NEGATIVE_SLOPE = 0.3  # PR #1089 uses 0.3
_SLOPE_SQ = _NEGATIVE_SLOPE ** 2  # Derived, stays in sync if slope changes
try:
    import triton
    ...
    def _fused_leaky_relu_sq_kernel(a_desc, b_desc, c_desc, aux_desc,
        ...
        for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
            ...
            if not FORWARD:
                ...
                # d(leaky_relu(z,s)^2)/dz = where(z>0, 2z, 2*s*s*z)
                c0 = c0 * tl.where(c0_pre > 0, 2.0 * c0_pre, 2.0 * NEG_SLOPE_SQ * c0_pre)
            c_desc.store([offs_am_c, offs_bn_c], c0)
            if FORWARD:
                c0_post = tl.where(c0 > 0, c0, NEG_SLOPE * c0)
                c0_post = c0_post * c0_post
                aux_desc.store([offs_am_c, offs_bn_c], c0_post)
            c1 = acc1.to(dtype)
            if not FORWARD:
                c1_pre = aux_desc.load([offs_am_c, offs_bn_c + BLOCK_SIZE_N // 2])
                c1 = c1 * tl.where(c1_pre > 0, 2.0 * c1_pre, 2.0 * NEG_SLOPE_SQ * c1_pre)
            c_desc.store([offs_am_c, offs_bn_c + BLOCK_SIZE_N // 2], c1)
            if FORWARD:
                c1_post = tl.where(c1 > 0, c1, NEG_SLOPE * c1)
    ...
    def _fused_leaky_relu_sq(a, b, aux=None):
        ...
        _fused_leaky_relu_sq_kernel[grid](
            ...
            NEG_SLOPE=_NEGATIVE_SLOPE, NEG_SLOPE_SQ=_SLOPE_SQ,
    ...
    class FusedUpActivation(torch.autograd.Function):
        """Fused forward: Triton matmul+leaky_relu+square. Standard backward: cuBLAS.
        PR #1105 showed that Triton backward causes eager mode (2.7x slower).
        This keeps backward as standard ops so torch.compile can fuse across layers."""
        @staticmethod
        def forward(ctx, x, up_w):
            x_flat = x.reshape(-1, x.shape[-1])
            pre, post = _fused_leaky_relu_sq(x_flat, up_w)
            # Pre-compute activation gradient for backward (PR #1105 trick)
            # act_grad = d(leaky_relu(z)^2)/dz = where(z>0, 2z, 2*s^2*z)
            act_grad = torch.where(pre > 0, 2.0 * pre, (2.0 * _SLOPE_SQ) * pre)
            ctx.save_for_backward(x_flat, up_w, act_grad)
            ctx.input_shape = x.shape
            return post.view(x.shape[:-1] + (post.shape[-1],))
        @staticmethod
        def backward(ctx, grad_post):
            x_flat, up_w, act_grad = ctx.saved_tensors
            gp = grad_post.reshape(-1, grad_post.shape[-1])
            # All standard PyTorch ops — torch.compile can fuse these
            dpre = gp * act_grad          # element-wise
            dW_up = dpre.T @ x_flat       # cuBLAS matmul
            dx = dpre @ up_w              # cuBLAS matmul
            return dx.reshape(ctx.input_shape), dW_up

    HAS_FUSED_MLP = True
    torch._dynamo.allow_in_graph(FusedUpActivation)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        _qb = CastedLinear._qat_default_bits
        up_w_q = _apply_bank_qat(up_w, _qb, x.dtype)
        down_w_q = _apply_bank_qat(down_w, _qb, x.dtype)
        if HAS_FUSED_MLP and x.is_cuda and not IS_ROCM:
            # Fused forward only (Triton) — backward uses standard ops (PR #1105 insight)
            post = FusedUpActivation.apply(x, up_w_q)
            return F.linear(post, down_w_q)
        x = F.leaky_relu(F.linear(x, up_w_q), negative_slope=_NEGATIVE_SLOPE)
        return F.linear(x.square(), down_w_q)
...
class _HessianMLP(nn.Module):
    ...
    def forward(self, x):
        return self.proj(F.leaky_relu(self.fc(x), negative_slope=_NEGATIVE_SLOPE).square())
